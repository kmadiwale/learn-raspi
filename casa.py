...
while True:
	...
	if x==1:
		...
		notify(config.get('mail','se_id'), 
			config.get('mail','re_id'), 
			config.get('mail', 'se_sub'),
			config.get('mail', 'se_pro'),
			config.get('mail', 'se_pro_port'),
			config.get('mail', 'se_phrase'),
...
def click(f, pir_id):
	print("Intrusion detected by " + pir_id + ", capturing image: " + f)
